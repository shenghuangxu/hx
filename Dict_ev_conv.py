...
kernel_size_set=(range(1,8,2))
for pad,ks in enumerate(kernel_size_set):
  for cout in C_out_set:
    # for pad in range(0,ks):
    #   for st in range(1,ks+1):
        # for dila in range(1,3):
          ops[K]=((lambda C_in,cout=cout,st=1,pad=pad,ks=ks:nn.Conv2d(in_channels=C_in,out_channels=cout  ,kernel_size=ks, stride=st, padding= pad)))
...
out_features_set=range(100,100)
 ...
 ##########################################################################################################################################################################
        ...
        if 'in_f'  in ops[X[i][index]].__code__.co_varnames:
          ...
          try:
            ...
            temp_cin=ops[j](C_in=input_c).out_channels
            model[i].append(nn.BatchNorm2d(temp_cin))
            oux=nn.BatchNorm2d(temp_cin)(oux)
            model[i].append(nn.ReLU(inplace=True))
            oux=nn.ReLU(inplace=True)(oux)
            temp_cin=ops[j](C_in=input_c).out_channels
          except:
